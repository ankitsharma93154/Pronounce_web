...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => ![PRECACHE, RUNTIME_CACHE, `precache-${CACHE_VERSION}`].includes(key))
...
self.addEventListener('fetch', (event) => {
  ...
  const isDataRoute = url.pathname.startsWith('/data/') || url.pathname.endsWith('.json');
  ...
  if (isDataRoute && isSameOrigin) {
    ...
    event.respondWith(
      caches.open(RUNTIME_CACHE).then(async (cache) => {
        ...
        return cached || networkPromise.then((res) => res || cached) || new Response(null, { status: 504 });
      })
    );
    return;
  }
